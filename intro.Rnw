...
\textsf{survey} est également capable de produire des
...
\begin{center}
  \begin{tabular}{>{\raggedright}p{2cm}>{\raggedright}p{3.5cm}>{\raggedright}p{2.5cm}>{\raggedright}p{2.5cm}>{\raggedright}p{2.5cm}}
    ...
    ACM & plusieurs variables qualitatives & \rfunc{dudi.acm} (\textsf{MASS}) & \rfunc{dudi.acm} & \rfunc{MCA} \tabularnewline
  \end{tabular}
\end{center}
...
répartissent les modalités de chaque variable sur un axe donné. Voir les figures
...
\begin{astuce}
  ...
  Une représentation classique des observations avec \rfubnc{s.label} ne permettra 
...
<<>>=
acm <- MCA(dt, ncp=5, graph=FALSE)
acm
acm$eig
sum(acm$eig$eigenvalue)
@
...
<<>>=
acm <- MCA(dt[complete.cases(dt),], ncp=5, graph=FALSE)
acm$eig
sum(acm$eig$eigenvalue)
@

Les possibilités graphiques de \rfunc{FactoMineR} sont plus limitées que celles
de \rfunc{ade}.
