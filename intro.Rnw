...
effets de chaque variable du modèle. Pour cela, il faut appliquer \rfunc{plot}
au résultat de la fonction \rfunc{allEffects}. Attention~: dans le cas présent
le graphique ne s'affichera pas correctement car la fonction \rfunc{allEffects}
ne sait pas gérer correctement les facteurs surlesquels on a appliqué \rfunc{addNA}.
En effet, \rfunc{addNA} créé une nouvelle modalité qui a pour nom la valeur manquante
elle-même et non une chaine de caractères. Nous allons donc donner un nom textuel
à la modalité <<~valeur manquante~>> et recalculer notre régression logistique
avant d'appeler \rfunc{allEffects}.

<<>>=
levels(d$etud)[is.na(levels(d$etud))] <- "Manquant"
reg <- glm(sport ~ sexe + grpage + etud + relig + heures.tv, 
		   data=d, family=binomial(logit))
@

Nous obtenons alors la figure \ref{fig_alleffects_reg} \vpageref{fig_alleffects_reg}.
